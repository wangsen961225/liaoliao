...
public class PushTest extends BaseTest {
     ...
     */
    @Test
    public void testPushMessageToAndroid(String title,String body) throws Exception {
     ...
     */
    @Test
    public void testPushNoticeToAndroid(String title,String body) throws Exception {
//               Map map=new HashMap();
//               map.put("1", "1");
...
//        androidRequest.setExtParameters(json);
     ...
     * //
     */
    @Test  //类型:消息 或通知 title :标题  map:信息  time:存活的时间
    public void testAdvancedPush(String message,String title,Map<String, String> extras,int time) throws Exception {
...
//        pushRequest.setTargetValue(deviceIds); //根据Target来设定，如Target=DEVICE, 则对应的值为 设备id1,设备id2. 多个值使用逗号分隔.(帐号与设备有一次最多100个的限制)
        ...
        // 推送配置
        pushRequest.setTitle("1"); // 消息的标题
        pushRequest.setBody("{\"k1\":\"android\",\"k2\":\"v2\"}"); // 消息的内容
...
//         推送配置: iOS
        ...
        ("{\"_ENV_\":\"DEV\",\"k2\":\"v2\"}"); //通知的扩展属性(注意 : 该参数要以json map的格式传入,否则会解析出错)
        // 推送配置: Android
        ...
        pushRequest.setAndroidXiaoMiNotifyTitle("1");
        pushRequest.setAndroidXiaoMiNotifyBody("1");
        ...
        System.out.println(client+"================"+pushRequest.getRegionId());
